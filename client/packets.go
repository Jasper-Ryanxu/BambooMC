...
func (c *Client) SendBlockUpdate(pos pk.Position, stateID int32) {
	c.SendPacket(packetid.ClientboundBlockUpdate, pos, pk.VarInt(stateID))
}
...
func (c *Client) SendGameEvent(event pk.UnsignedByte, value float32) {
	c.SendPacket(packetid.ClientboundGameEvent, event, pk.Float(value))
}

func (c *Client) SendWorldTime(time, dayTime int64) {
	c.SendPacket(packetid.ClientboundSetTime, pk.Long(time), pk.Long(dayTime))
}
...
func (c *Client) SendBlockChangedAck(sequence int32) {
	c.SendPacket(packetid.ClientboundBlockChangedAck, pk.VarInt(sequence))
}

func (c *Client) SendAddItemEntity(e *world.ItemEntity) {
	c.SendPacket(
		packetid.ClientboundAddEntity,
		pk.VarInt(e.EntityID),
		pk.UUID(e.EntityIDAsUUID()),
		pk.VarInt(world.ItemEntityTypeID),
...
func (c *Client) SendSetEntityData(id int32, items []world.EntityDataItem) {
	var buf bytes.Buffer
	_, _ = pk.VarInt(id).WriteTo(&buf)
	for _, item := range items {
		_, _ = item.Index.WriteTo(&buf)
		_, _ = item.Type.WriteTo(&buf)
		_, _ = item.Value.WriteTo(&buf)
	}
	buf.WriteByte(0xFF) // end of metadata
	c.queue.Push(pk.Packet{
		ID:   int32(packetid.ClientboundSetEntityData),
		Data: buf.Bytes(),
	})
}

func (c *Client) SendTakeItemEntity(itemID, playerID int32, count pk.VarInt) {
	c.SendPacket(packetid.ClientboundTakeItemEntity, pk.VarInt(itemID), pk.VarInt(playerID), count)
}

func (c *Client) ViewChunkLoad(pos level.ChunkPos, chunk *level.Chunk)    { c.SendLevelChunkWithLight(pos, chunk) }
func (c *Client) ViewChunkUnload(pos level.ChunkPos)                       { c.SendForgetLevelChunk(pos) }
func (c *Client) ViewBlockUpdate(pos pk.Position, stateID int32)          { c.SendBlockUpdate(pos, stateID) }
func (c *Client) ViewAddPlayer(p *world.Player)                           { c.SendAddPlayer(p) }
func (c *Client) ViewAddItemEntity(e *world.ItemEntity)                   { c.SendAddItemEntity(e) }
...
func (c *Client) ViewSetEntityData(id int32, items []world.EntityDataItem) {
	c.SendSetEntityData(id, items)
}

func (c *Client) ViewTakeItemEntity(itemID, playerID int32, count pk.VarInt) {
	c.SendTakeItemEntity(itemID, playerID, count)
}

func (c *Client) ViewWorldTime(time, dayTime int64) {
	c.SendWorldTime(time, dayTime)
}
